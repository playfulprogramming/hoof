...
export async function fetchAsBrowser(input: string | URL, init?: RequestInit) {
	...
	if (!response.ok) throw new Error(`Request ${input} returned ${response.status}`);
	return response;
}
...
export async function getOpenGraphImage(root: Root, baseUrl: URL): Promise<URL> {
	const metaNode = find<Element>(root, (e) =>
		isElement(e) &&
		e.tagName === "meta" &&
